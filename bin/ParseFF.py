    ...
    with open(ff_file,"r") as f:
        for line in f:
            ...
            if da_line[0]=="vdw":
                ...
                if ff.rad_type=="R-MIN":val*=2.0
                ep=float(da_line[3])*kcalmol2au
                ffterm=(models["lj"],intcoords["pair14"])
                ff.paramtypes[ffterm]=typetypes["class"]
                ff.terms[ffterm]="LJ14"
                check_add(ff.params,ffterm,params["sigma"],elem,val)
                check_add(ff.params,ffterm,params["epsilon"],elem,ep)
                ffterm=(models["lj"],intcoords["pair"])
                ff.paramtypes[ffterm]=typetypes["class"]
                ff.terms[ffterm]="LJPair"
                check_add(ff.params,ffterm,params["sigma"],elem,val)
                check_add(ff.params,ffterm,params["epsilon"],elem,ep)
    return ff
...
def main():
    ...
    f.write("const FManII::ForceField FManII::"+ff_name+"={\n")
    print_parms(f,ff_name,ff.params)
    f.write("{\n")
    for key,value in ff.type2class.items():
        f.write("{"+str(key)+","+str(value)+"},\n")
    f.write("},\n{\n")
    for key,value in ff.terms.items():
        f.write("{{"+key[0]+","+key[1]+"},FManII::"+value+"()},\n")
    f.write("},\n{\n")
    for key,value in ff.orderrules.items():
        f.write("{{"+key[0]+","+key[1]+"},"+value+"},\n")
    f.write("},\n{\n")
