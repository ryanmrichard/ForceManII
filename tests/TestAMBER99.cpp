...
int main(int argc, char** argv){
    ...
    test_header("Testing derivatives of pre-packaged AMBER99 force-fields");
    auto deriv=run_forcemanii(0,ubiquitin,ubiquitin_conns,amber99,
                                    ubiquitin_FF_types);
    for(auto derivi : deriv)
    {
        const auto ffterm=derivi.first;
        const string msg=string("AMBER99")+" "+ffterm.first+" "+ffterm.second+" energy";
        double tol=1e-5;
    ...
    deriv=run_forcemanii(1,ubiquitin,ubiquitin_conns,amber99,
                                    ubiquitin_FF_types);
    for(auto derivi : deriv)
    {
        const auto ffterm=derivi.first;
