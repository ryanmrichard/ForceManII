...
inline void no_param_error(const std::string& parami,
                           const IVector& types,
                           bool use_class){
    stringstream ss;
...
Vector ForceField::assign_param(const FFTerm_t& term_type,
    ...
    for(auto typei:coord.get_types()){//Loop over types of coords in term
        ...
        if(combrules.count(prule)){//Has combining rule?
            ...
            for(auto ti:types){
                const Vector vs=
                        params.get_param(term_type,parami,IVector({ti}));
                if(vs.size()==1)in_params.push_back(vs[0]);
        ...
        const bool is_vector=term_type.first==Model_t::FOURIERSERIES &&
            ...
            for(size_t i=1;i<n;++i){
                ...
                do{
                    ...
                    for(size_t j=0;j<n;++j)
                        if(is_zero[j])temp[j]=wild_card;
                    vs=params.get_param(term_type,parami,orderrules.at(term_type)(temp));
                    if(vs.size())break;
